...
def load_img_angular_velocity(datadir, df):
    ...
    for i in range(len(df)):
        ...
        # flipped = np.random.rand() > 0.0
        # if flipped:
        #     angle *= -1  # Flip the angle for mirrored image
        ...
        # if any(cam in image_file for cam in ["camera_pagi", "camera_siang", "camera_sore"]):
        #     if angle <= -0.1:
        #         angle += -0.05
        #     elif angle >= 0.1:
        #         angle += 0.05
...
def cil_model_improved_with_attention():
    ...
    # Attention mechanism
    attention = Conv2D(64, kernel_size=(3, 3), activation='relu', padding='same')(model.output)
    ...
    # Apply attention to the model
    model_output = Multiply()([model.output, attention])
